...
def run_install(tmp_path: Path, *, scope: str = "project") -> subprocess.CompletedProcess:
    ...
    if scope == "project":
        return subprocess.run(args, cwd=str(tmp_path), capture_output=True, text=True, env=env)
    return subprocess.run(args, cwd=str(tmp_path), capture_output=True, text=True, env=env)
